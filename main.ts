...
function call_dot_at([x, y]: [number, number]): string {
    return `<path d="m${x} ${y}v3a3 3 0 0 0 0-6 3 3 0 0 0 0 6v-3" stroke="#800080" />`;
}

function call_vowel_anchored_at(vowel: Vowel, [x, y]: [number, number]): string {
...
function call_vowel_non_anchored(vowel: Vowel, consonant: Consonant): string {
...
function automatic({ consonant, vowel, dotted }: { consonant: Consonant, vowel: Vowel | 'a', dotted: boolean }): string[] {
